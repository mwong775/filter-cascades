...
data = np.genfromtxt('level_lf.csv',delimiter=',', dtype = float)
print(data)

lvl = [row[0] for row in data]
print(lvl)
cfc = [row[1] for row in data]
cfc = [x for x in cfc if x != 0]
print(cfc)
cfssc = [row[2] for row in data]
cfssc = [x for x in cfssc if x != 0]
print(cfssc)
vfc = [row[3] for row in data]
vfc = [x for x in vfc if x != 0]
print(vfc)
vfssc = [row[4] for row in data]
...
plt.ylabel("Load Factor (%)", fontsize = 24)
plt.savefig("level_lf.png")
plt.show()
