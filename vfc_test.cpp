...
void test_vfc(vector<uint64_t> r, vector<uint64_t> s, vector<uint64_t> fp, FILE *file) {
    ...
    cout << "start vfc insert" << endl;
    auto start = chrono::steady_clock::now();  
    vfc.insert(r, s, fp, file);
    auto end = chrono::steady_clock::now();
    cout << "finish vfc insert" << endl;
    double cost = time_cost(start, end);
    vfc.print_cascade();

    cout << "vfc insert time: " << cost << endl;
    fprintf(file, "vfc insert time\n");
    fprintf(file, "%f\n", cost);

    cout << "start vfc lookup" << endl;
    ...
    cout << "finish vfc revoked lookup " << i << endl;
    ...
    cout << "finish vfc unrevoked lookup " << j << endl;
    cout << "false pos: " << e2 << endl;
    cost = time_cost(start, end);
    cout << "vfc lookup time: " << cost << endl;
