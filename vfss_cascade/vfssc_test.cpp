...
void test_vfc(vector<uint64_t> r, vector<uint64_t> s, vector<uint64_t> fp, FILE *file) {
    VFSSCascade vfssc;
    fprintf(file, "Vacuum Filter Cascade\n");
    fprintf(file, "level, insert, n, load factor, lookup, # fp's, fp, memory(bytes), bits per item\n");
    cout << "start vfssc insert" << endl;
    auto start = chrono::steady_clock::now();  
    vfssc.insert(r, s, fp, file);
    auto end = chrono::steady_clock::now();
    cout << "finish vfssc insert" << endl;
    double cost = time_cost(start, end);
    vfssc.print_cascade();

    cout << "vfssc insert time: " << cost << endl;
    fprintf(file, "vfssc insert time\n");
    fprintf(file, "%f\n", cost);

    cout << "start vfssc lookup" << endl;
    ...
    for(auto c : r) {
        if(!vfssc.lookup(c)) {
            if(e1 == 0)
                cout << "VFC ERROR ON REVOKED " << c << " index " << i << endl;
            e1++;
        }
        i++;
    }
    cout << "finish vfssc revoked lookup " << i << endl;
    ...
    for(auto d : s) {
        if(vfssc.lookup(d)) {
            if(e2 == 0)
                cout << "VFC ERROR ON UNREVOKED " << d << " index " << j << endl;
            e2++;
        }
    j++;
    }
    end = chrono::steady_clock::now();
    cout << "finish vfssc unrevoked lookup " << j << endl;
    cout << "false pos: " << e2 << endl;
    cost = time_cost(start, end);
    cout << "vfssc lookup time: " << cost << endl;
    fprintf(file, "vf lookup time\n");
    fprintf(file, "%f\n", cost);
}
