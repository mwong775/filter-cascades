...
class SingleTable {
 ...
 public:
  explicit SingleTable(const size_t num) : num_buckets_(num) {
    ...
    // cout << "num = " << num << endl;
    // cout << "num buckets = " << num_buckets_ << endl;
    // cout << "kPaddingBuckets: " << kPaddingBuckets << endl;
  }
  ...
  std::string Info() const {
    ...
    ss << "\t\tTotal # of rows (buckets): " << num_buckets_ << "\n";
    ss << "\t\tTotal # slots (assoc. * rows): " << SizeInTags() << "\n";
    return ss.str();
  }
  ...
  inline bool InsertTagToBucket(const size_t i, const uint32_t tag,
                                const bool kickout, uint32_t &oldtag, uint32_t *tags) {
    for (size_t j = 0; j < kTagsPerBucket; j++) {
      tags[j] = ReadTag(i, j);
      if (tags[j] == 0) {
        WriteTag(i, j, tag);
