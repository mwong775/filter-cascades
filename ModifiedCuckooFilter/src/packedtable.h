...
class PackedTable {
 ...
 public:
  explicit PackedTable(size_t num) : num_buckets_(num) {
    ...
    // cout << "BITS_PER_TAG: " << bits_per_tag << endl;
    // cout << "KBITSPERBUCKET: " << kBitsPerBucket << endl;
  }
  ...
  inline void ReadBucket(const size_t i, uint32_t tags[4]) const {
    ...
    if (bits_per_tag == 5) {
      ...
      // cout << "9 dirbits per tag,  48 bits per bucket" << endl;
      p = buckets_ + (i * 6);
      ...
      // cout << "13 dirbits per tag, 64 bits per bucket" << endl;
      p = buckets_ + (i << 3);
      uint64_t bucketbits = *((uint64_t *)p);
      codeword = *((uint16_t *)p) & 0x0fff;
      tags[0] = (bucketbits >> 8) & kDirBitsMask;
